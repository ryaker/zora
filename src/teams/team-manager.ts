...
export class TeamManager {
  ...
  async addMember(teamName: string, member: Omit<AgentMember, 'isActive'>): Promise<void> {
  ...
  async updateMemberStatus(teamName: string, agentId: string, isActive: boolean): Promise<void> {
    const config = await this.getTeam(teamName);
    if (!config) throw new Error(`Team "${teamName}" not found`);
